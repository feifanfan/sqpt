...
class ShippingController extends CommonController{
	...
	public function templates()
	{
		...
		$list = M()->query('SELECT * FROM ' . C('DB_PREFIX') . 'lionfish_comshop_shipping WHERE 1 ' . 
				$condition . '  ORDER BY sort_order DESC limit ' . (($pindex - 1) * $psize) . ',' . $psize);
		
		$total = M('lionfish_comshop_shipping')->where( "1 ". $condition )->count();
	...
	public function setdefault()
	{
		...
		if ($_GPC['isdefault'] == 1) {
			M('lionfish_comshop_shipping')->where( 1 )->save( array('isdefault' => 0) );
		}
		
		$items = M('lionfish_comshop_shipping')->field('id,name')->where( 'id in( ' . $id . ' )' )->select();

		foreach ($items as $item) {
			
			M('lionfish_comshop_shipping')->where( array('id' => $item['id']) )->save( array('isdefault' => intval($_GPC['isdefault'])) );
	...
	public function enabled()
	{
		...
		$items = M('lionfish_comshop_shipping')->where( ' id in( ' . $id . ' ) ' )->select();	
			
		foreach ($items as $item) {
			
			M('lionfish_comshop_shipping')->where( array('id' => $item['id'])  )->save( array('enabled' => intval($_GPC['enabled'])) );
		}
	...
	public function deleteshipping()
	{
		...
		foreach ($items as $item) {
			...
			M('lionfish_comshop_shipping')->where( array('id' => $item['id']) )->delete();
	...
	public function editshipping()
	{
		...
		$item = M('lionfish_comshop_shipping')->where( array('id' => $id) )->find();
