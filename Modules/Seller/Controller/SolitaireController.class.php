...
class SolitaireController extends CommonController{
	...
	public function index()
	{
        ...
        $list = M()->query('SELECT * FROM ' . C('DB_PREFIX'). "lionfish_comshop_solitaire  
		...
		foreach( $list as $key => $val )
		{
			...
			$head_info = M('lionfish_community_head')->field('community_name,head_name')->where( array('id' => $val['head_id'] ) )->find();
			...
			$goods_count = M('lionfish_comshop_solitaire_goods')->where( array('soli_id' => $val['id'] ) )->count();
		...
		$total = M('lionfish_comshop_solitaire')->where( '1 '.$condition )->count();
		...
		$all_count = M('lionfish_comshop_solitaire')->count();
		...
		//进行中的
		$count_status_1 = M('lionfish_comshop_solitaire')->where( "appstate=1 and state=1 and begin_time <={$now_time} and end_time >{$now_time} " )->count();
		...
		//未开始（{$count_status_2}
		$count_status_2 = M('lionfish_comshop_solitaire')->where( "appstate=1 and state=1 and begin_time >{$now_time} " )->count();
		...
		//已结束（{$count_status_3}）
		$count_status_3 = M('lionfish_comshop_solitaire')->where("appstate=1 and state=1 and end_time <={$now_time} ")->count();
		...
		//未审核（{$count_status_4}
		$count_status_4 = M('lionfish_comshop_solitaire')->where("appstate=0")->count();
		...
		$count_status_5 = M('lionfish_comshop_solitaire')->where("appstate=2")->count();
	...
	public function detail()
	{
		...
		$head_info = M('lionfish_community_head')->where( array('id' => $solitaire_info['head_id'] ) )->find();
		
		$this->head_info = $head_info;
		
		$soli_goods = M('lionfish_comshop_solitaire_goods')->field('goods_id')->where( array('soli_id' => $id ) )->select();
		...
		if( !empty($soli_goods) )
		{
			...
			$sql = "select g.id,g.goodsname,g.codes,g.price,productprice,total from ".C('DB_PREFIX')."lionfish_comshop_goods as g , ".C('DB_PREFIX')."lionfish_comshop_good_common as gc 
	...
	public function add()
	{
        ...
        if (!empty($id)) {
			...
			$item = M('lionfish_comshop_solitaire')->where( array('id' => $id ) )->find();
			...
			$headinfo = M('lionfish_community_head')->where( array('id' => $item['head_id'] ) )->find();
			...
			$sql = "select g.id as gid, g.goodsname  from ".C('DB_PREFIX')."lionfish_comshop_solitaire_goods as gs , ".C('DB_PREFIX')."lionfish_comshop_goods as g  
					where gs.soli_id={$id} and gs.goods_id=g.id  ";
