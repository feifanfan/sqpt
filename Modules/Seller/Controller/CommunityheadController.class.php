...
class CommunityheadController extends CommonController {
	...
	public function index(){
		...
		$sql = 'SELECT ch.*,m.we_openid,m.username,m.avatar FROM ' . C('DB_PREFIX') . "lionfish_community_head as ch left join  
					".C('DB_PREFIX')."lionfish_comshop_member as m on  ch.member_id = m.member_id                
		...
		$sql_count = 'SELECT count(1) as total FROM ' . C('DB_PREFIX') . 'lionfish_community_head as ch  
						left join '.C('DB_PREFIX').'lionfish_comshop_member as m on ch.member_id = m.member_id 
						WHERE 1  ' . $condition;
		...
		$all_sell_count = M('lionfish_comshop_goods')->where( array('is_all_sale' => 1) )->count();
		...
		$group_list = M('lionfish_community_head_group')->order('id asc')->select();
		...
		foreach( $list as $key => $val )
		{
			...
			$commission_info = M('lionfish_community_head_commiss')->where( array('head_id' => $val['id'],'member_id' => $val['member_id'] ) )->find();


			//预计佣金 commission_total
			$pre_total_money = M('lionfish_community_head_commiss_order')->where( array('state' => 0, 'head_id' => $val['id'] ) )->sum('money');
			...
			{
				$parent_community_head = M('lionfish_community_head')->field('head_name')->where( array('id' => $val['agent_id'] ) )->find();


				$val['agent_name'] = $parent_community_head['head_name'];
			}

			$member_info = M('lionfish_comshop_member')->field('username,avatar,we_openid')->where( array('member_id' => $val['member_id'] ) )->find();
			...
			//团长商品
			$head_goods_count = M()->query("select count(hg.id) as count from ".C('DB_PREFIX')."lionfish_community_head_goods as hg ,".C('DB_PREFIX')."lionfish_comshop_good_common as gc ,".C('DB_PREFIX')."lionfish_comshop_goods as g 
								where hg.goods_id = gc.goods_id and gc.goods_id = g.id and g.is_all_sale=0  and hg.head_id = ". $val['id']  );

			$val['head_goods_count'] = 	$head_goods_count[0]['count'];
			//所有团长可售商品
			...
			$val['goods_count'] += $all_sell_count;


			//团长订单
			$val['head_order_count'] = M('lionfish_comshop_order')->where( array('head_id' => $val['id'] ) )->count();



			$member_count_arr = M()->query("SELECT count(DISTINCT(member_id) ) as count  FROM ".C('DB_PREFIX')."lionfish_community_history WHERE head_id =". $val['id']);
		...
		if ($export == '1') {
			...
			foreach ($list as &$row) {
			    ...
			    $row['getmoney'] = $row['pre_total_money'];
	...
	public function lineheadquery()
	{
		...
		$ds = M()->query('SELECT * FROM ' . C('DB_PREFIX') . 'lionfish_community_head WHERE 1 ' . $condition . ' order by id asc');

		$had_head_list = M()->query('select head_id from '.C('DB_PREFIX')."lionfish_comshop_deliveryline_headrelative ");
	...
	public function usergroup()
	{
		...
		$alllist = M('lionfish_community_head_group')->where( "1 ". $condition  )->order('id asc')->select();

		foreach ($alllist as &$row ) {

			$sql = 'select count(*) as count from ' . C('DB_PREFIX') .'lionfish_community_head where  find_in_set('.$row['id'].',groupid) limit 1';
	...
	public function deleteusergroup()
	{
		...
		$items = M('lionfish_community_head_group')->field('id,groupname')->where( 'id in( ' . $id . ' )' )->select();

		foreach ($items as $item ) {

			M('lionfish_community_head')->where( array('groupid' => $item['id'] ) )->save( array('groupid' => 0) );

			M('lionfish_community_head_group')->where( array('id' => $item['id']) )->delete();
	...
	public function enable_check()
	{
		...
		$members = M('lionfish_community_head')->field('id,member_id,enable')->where( array('id' =>array('in', $id)) )->select();
		...
		foreach ($members as $member) {
			...
			if ($comsiss_state == 1) {
				...
				M('lionfish_community_head')->where( array('id' => $member['id']) )->save( array('enable' => 1) );
			}
			else {
				M('lionfish_community_head')->where( array('id' => $member['id']) )->save( array('enable' => 0) );
			}
		}
	...
	public function deletecommunitymember()
	{
		...
		$apply_info = M('lionfish_comshop_community_pickup_member')->where( array('id' => $id ) )->find();

		M('lionfish_comshop_member')->where( array('member_id' => $apply_info['member_id']) )->save( array('pickup_id' => 0 ) );

		M('lionfish_comshop_community_pickup_member')->where( array('id' => $id ) )->delete();
	...
	public function agent_check_communitymember()
	{
		...
		$apply_list = M('lionfish_comshop_community_pickup_member')->where( 'id in( ' . $id . ' )' )->select();


		foreach ($apply_list as $apply) {

			M('lionfish_comshop_community_pickup_member')->where( array('id' => $apply['id'])  )->save( array('state' => $state ) );
	...
	public function communityhead()
	{
		...
		$sql = 'SELECT ch.*,m.we_openid,m.username,m.avatar FROM ' . C('DB_PREFIX') . "lionfish_community_head as ch left join ".C('DB_PREFIX')."lionfish_comshop_member as m on  ch.member_id = m.member_id                 
						WHERE  1 " . $condition . ' order by ch.id desc  ';
		...
		$sql_count = 'SELECT count(1) as count FROM ' . C('DB_PREFIX'). 'lionfish_community_head as ch  left join  '.C('DB_PREFIX').'lionfish_comshop_member as m  on ch.member_id = m.member_id 
						WHERE  1 ' . $condition;
		...
		$community_head_group = M('lionfish_community_head_group')->order('id asc')->select();
		...
		foreach( $list as $key => $val )
		{
			...
			$head_goods_count_arr = M()->query("select count(hg.id) as count from ".C('DB_PREFIX')."lionfish_community_head_goods as hg ,".C('DB_PREFIX')."lionfish_comshop_good_common as gc ,".C('DB_PREFIX')."lionfish_comshop_goods as g 
								where hg.goods_id = gc.goods_id and gc.goods_id = g.id and g.is_all_sale=0 and hg.head_id =".$val['id']  );
			...
			$member_count_arr = M()->query("SELECT count(DISTINCT(member_id) ) as count  FROM ".
									C('DB_PREFIX')."lionfish_community_history WHERE head_id =".$val['id']);

			$val['member_count'] =	$member_count_arr[0]['count'];

			$val['agent_count'] = M('lionfish_community_head')->where( array('agent_id' => $val['id'] ) )->count();
	...
	public function changelevel()
	{
		...
		if($toggle == 'group')
		{
			M('lionfish_community_head')->where( "id in ({$ids})" )->save( array('groupid' => $level) );
		}else if($toggle == 'level'){
			M('lionfish_community_head')->where("id in ({$ids})")->save( array('level_id' => $level ) );
		}
	...
	public function deletehead()
	{
		...
		$items = M('lionfish_community_head')->field('id')->where( 'id in( ' . $id . ' )' )->select();

		foreach ($items as $item ) {
			M('lionfish_community_head')->where( array('id' => $item['id'] ) )->delete();
		}
	...
	public function look_piup_record()
	{
		...
		$sql = 'SELECT * FROM ' . C('DB_PREFIX') ."lionfish_comshop_community_pickup_member_record                
		...
		$total = M('lionfish_comshop_community_pickup_member_record')->where( $condition )->count();
	...
	public function lookcommunitymember()
	{
		...
		$sql = 'SELECT pm.*, m.username FROM ' . C('DB_PREFIX'). "lionfish_comshop_community_pickup_member as pm ,  ".
		...
		foreach($list as $key => $val)
		{
			...
			$he_count = M('lionfish_comshop_community_pickup_member_record')->where( array('member_id' => $val['member_id'] ) )->count();
		...
		$total_arr = M()->query('SELECT count(1) as count FROM ' . C('DB_PREFIX'). 'lionfish_comshop_community_pickup_member as pm , '.
					C('DB_PREFIX').'lionfish_comshop_member as m  WHERE pm.member_id = m.member_id  ' . $condition);
	...
	public function goodslist()
	{
		...
		$all_sales_goods = M('lionfish_comshop_goods')->field('id')->where( array('is_all_sale' => 1) )->select();
		...
		$ch_goods_list = M('lionfish_community_head_goods')->field('goods_id')->where( array('head_id' => $head_id) )->select();
		...
		$sql = 'SELECT COUNT(g.id) as count FROM ' .C('DB_PREFIX') . 'lionfish_comshop_goods as g   '
				." where {$where}  " ;
		...
		if (!(empty($total))) {
			...
			$sql = 'SELECT g.* FROM ' . C('DB_PREFIX') . 'lionfish_comshop_goods as g ' .
	...
	public function down_sales()
	{
		...
		if(!empty($id))
		{
			M('lionfish_community_head_goods')->where( array('head_id' => $head_id, 'goods_id' => array('in', $id)) )->delete();
		}
	...
	public function rest_check()
	{
		...
		$members = M('lionfish_community_head')->field('id,member_id,enable')->where(  'id in( ' . $id . ' )' )->select();
		...
		foreach ($members as $member) {
			...
			if ($comsiss_state == 1) {
				M('lionfish_community_head')->where(  array('id' => $member['id']) )->save( array('rest' => 1) );
			}
			else {
				M('lionfish_community_head')->where(  array('id' => $member['id']) )->save( array('rest' => 0) );
			}
		}
	...
	public function default_check()
	{
		...
		foreach ($members as $member) {
			...
			if ($is_default == 1) {
				...
				M('lionfish_community_head')->where( 'id>0' )->save( array('is_default' => 0) );

				M('lionfish_community_head')->where( array('id' => $member['id'] ) )->save( array('is_default' => 1)  );

			}
			else {

				M('lionfish_community_head')->where( array('id' => $member['id']  )  )->save(  array('is_default' => 0) );
			}
		}
	...
	public function distribulist()
	{
		...
		$sql = 'SELECT * FROM ' . C('DB_PREFIX')."lionfish_community_head_tixian_order                 
		...
		$total = M('lionfish_community_head_tixian_order')->where("1=1".$condition)->count();
		...
		foreach( $list as $key => $val )
		{
			...
			$val['community_head_commiss'] = M('lionfish_community_head_commiss')->where( array('head_id' =>$val['head_id'] ) )->find();
			...
			$val['community_head'] = M('lionfish_community_head')->where( array('id' => $val['head_id'] ) )->find();
	...
	public function agent_check_apply()
	{
		...
		$apply_list = M('lionfish_community_head_tixian_order')->where( array('id'=> array('in', $id) ) )->select();
			...
			if ($comsiss_state == 1) {
				...
				if( $apply['type'] > 0 )
				{
					if( $apply['type'] == 1 )
					{
						...
						$commiss_head_info = M('lionfish_community_head_commiss')->where( array('head_id' => $head_id,'member_id' =>$apply['member_id'] ) )->find();
						...
						if( !empty($open_weixin_qiye_pay) && $open_weixin_qiye_pay == 1 )
						{
								...
								$mb_info = M('lionfish_comshop_member')->field('we_openid')->where( array('member_id' =>$apply['member_id'] ) )->find();
					...
					//member_id
					$commiss_head_info = M('lionfish_community_head_commiss')->where( array('member_id' => $apply['member_id'],'head_id' => $head_id ) )->find();
				...
				M('lionfish_community_head_tixian_order')->where( array('id' => $apply['id']) )->save(  array('state' => 1,'service_charge' => $service_charge, 'shentime' => $time));

				//将冻结的钱划一部分到已提现的里面
				M()->execute("update ".C('DB_PREFIX')."lionfish_community_head_commiss set getmoney=getmoney+{$money},dongmoney=dongmoney-{$money} 
				...
				M('lionfish_community_head_tixian_order')->where( array('id' => $apply['id']) )->save( array('state' => 2, 'shentime' => $time) );

				//退回冻结的货款
				M()->execute("update ".C('DB_PREFIX')."lionfish_community_head_commiss set money=money+{$money},dongmoney=dongmoney-{$money} 
							where  head_id={$head_id} ");
			}
			else {
				M('lionfish_community_head_tixian_order')->where( array('id' => $member['id']) )->save( array('state' => 0, 'shentime' => 0) );
			}
		}
	...
	private function modifylevel()
	{
		...
		if ($id == 'default') {
			...
			$level = M('lionfish_comshop_community_head_level')->where( array('id' => intval($id) ) )->find();
		...
		if (IS_POST) {
			...
			if (!empty($id)) {
				if ($id == 'default') {
					...
					M('lionfish_comshop_community_head_level')->where( array('id' => $id) )->save( $data );
				}
			}
			else {
				$id = M('lionfish_comshop_community_head_level')->add( $data );
			}
	...
	public function headlevel()
	{
		...
		$alllist = M('lionfish_comshop_community_head_level')->where( "1 ".$condition) ->order('id asc')->select();
	...
	public function query_head_user_agent()
	{
		...
		**/
		...
		$ds = M()->query('SELECT m.*,ch.id  as head_id FROM ' . C('DB_PREFIX'). 'lionfish_comshop_member as m,'.C('DB_PREFIX').'lionfish_community_head as ch WHERE m.member_id=ch.member_id ' . $condition .
				' order by m.member_id asc' .' limit ' . (($page - 1) * $page_size) . ',' . $page_size );

		$total_arr = M()->query('SELECT count(1) as count FROM ' . C('DB_PREFIX') .
				'lionfish_comshop_member as m, '.C('DB_PREFIX').'lionfish_community_head as ch WHERE m.member_id=ch.member_id ' . $condition );
	...
	public function query_head_user()
	{
		...
		**/
		...
		$ds = M()->query('SELECT m.*,ch.id as head_id FROM ' . C('DB_PREFIX') . 'lionfish_comshop_member as m,'.C('DB_PREFIX').'lionfish_community_head as ch WHERE m.member_id=ch.member_id '
				. $condition .
				' order by m.member_id asc' .' limit ' . (($page - 1) * $page_size) . ',' . $page_size );
	...
	public function deletelevel()
	{
		...
		$items = M()->query('SELECT id FROM ' . C('DB_PREFIX') . 'lionfish_comshop_community_head_level WHERE id in( ' . $id . ' ) ' );

		foreach ($items as $item ) {

			M('lionfish_comshop_community_head_level')->where( array('id' => $item['id']) )->delete();

			M('lionfish_community_head')->where( array('level_id' => $item['id']) )->save( array('level_id' => 0 ) );
	...
	public function query_head()
	{
		...
		**/
		...
		$list = M('lionfish_community_head')->where( $where )->limit( (($page - 1) * $page_size) . ',' . $page_size )->select();

		$total = M('lionfish_community_head')->where( $where )->count();
		...
		{
			//ims_
			$member_info = M('lionfish_comshop_member')->field('username,avatar')->where( array('member_id' => $val['member_id']) )->find();
	...
	public function addhead()
	{
		...
		if (IS_POST) {
			...
			if( $id <=0  )
			{
				...
				$ck_head = M('lionfish_community_head')->where( array('member_id' => $_GPC['member_id'] ) )->find();
			...
			if( !empty($data['member_id']) && $data['member_id'] > 0 )
			{
				...
				$head_commiss_info = M('lionfish_community_head_commiss')->where( array('head_id' => $rs,'member_id' => $data['member_id']) )->find();
				...
				if( !empty($head_commiss_info) )
				{
					...
					M('lionfish_community_head_commiss')->where( array('id' => $head_commiss_info['id']) )->save( $commiss_data );
		...
		if($id > 0)
		{
			...
			if(!empty($item['agent_id']) && $item['agent_id'] > 0)
			{
				...
				$agent_saler = M('lionfish_comshop_member')->field('member_id, username as nickname,avatar')->where( array('member_id' => $agent_member_id ) )->find();
			}
	...
	public function agent_check()
	{
		...
		$members = M('lionfish_community_head')->field('id,member_id,state')->where( array('id' => array('in', $id ) ) )->select();
		...
		foreach ($members as $member) {
			...
			if ($comsiss_state == 1) {
				...
				M('lionfish_community_head')->where( array('id' => $member['id']) )->save( array('state' => 1, 'apptime' => $time) );
				...
				M('lionfish_community_head')->where( array('id' => $member['id']) )->save( array('state' => 2, 'apptime' => $time) );

			}
			else {

				M('lionfish_community_head')->where( array('id' => $member['id']) )->save( array('state' => 0, 'apptime' => 0) );
    ...
    public function addReward(){
        if(IS_POST){
            $param = I('post.');
            $info['amount'] = $param['amount'];
            //$info['percent'] = $param['percent'];
            $id = M('lionfish_community_head_reward')->where($info)->select(['id']);
            if(!empty($id)){
                show_json( -1,'金额重复');
            }
            M('lionfish_community_head_reward')->add($info);
            $status['status'] = 'success';
            $status['message'] = '操作成功';
            $status['jump'] = U('Communityhead/rewardList');
            show_json(1, array('url' => U('Communityhead/rewardList')));
        }
        $this->display('editheadreword');
    }
    public function editReward(){
        if(IS_POST){
            ...
            M('lionfish_community_head_reward')->where(['id'=>$id])->save($info);
            show_json(1, array('url' => U('Communityhead/rewardList')));

        }
        $reward['id'] = I('get.id');
        $reward = M('lionfish_community_head_reward')->where(['id'=>$reward['id']])->find(['id','amount','percent']);
        $this->assign('reward',$reward);
        $this->display('editheadreword');
    }
    /**
     * @description：激励计划搜索列表
     * @date:2020/5/6
     * @author ff
     */
	public function rewardList(){
        $list = M('lionfish_community_head_reward')->select();
    ...
    public function rewordheadlist(){
        ...
        foreach ($heads as $k=>&$v){
            //团长业绩、原提成、原提成比例
            $v['money'] = $this->getheadmoney($v['id'],$v['level_id'],[$starttime,$endtime]);
    ...
    //获取团长等级列表
    private function getjjlist(){
        //默认等级
        $head_commission_levelmoney = D('Home/Front')->get_config_by_name('default_comunity_money');
        //其他等级
        $x[] = [
            'id'=>0,
            'commission'=>$head_commission_levelmoney
        ];
        $level = M('lionfish_comshop_community_head_level')->field(['id','commission'])->select();
        $level = array_merge($x,$level);
        foreach ($level as $k=>$v){
            $ret[$v['id']]=$v['commission'];
        }
        return $ret;
    }
    //获取激励计划列表，按照超过金额排序
    private function getjililist(){
       return M('lionfish_community_head_reward')->order('amount')->field(['id','amount','percent'])->select();
    }
