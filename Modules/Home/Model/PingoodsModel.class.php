...
class PingoodsModel {
	...
	public function get_new_community_index_goods($head_id=0,$gid='', $fields='*', $where='1=1',$offset=0,$perpage=10,$order='g.istop DESC, g.settoptime DESC,g.index_sort desc,g.id desc ')
	{
		...
		if( $head_id > 0 )
		{
			$where .= " and (g.is_all_sale = 1 or g.id in (SELECT goods_id from ".C('DB_PREFIX')."lionfish_community_head_goods where head_id = {$head_id}) ) ";
		}
		
		if( !empty($gid) )
		{
			$gd_cate_sql  = " select goods_id from ".C('DB_PREFIX')."lionfish_comshop_goods_to_category where cate_id in ({$gid}) ";
	...
	**/
	public function get_commission_index_share_image($community_id,$wepro_qrcode,$avatar,$nickname)
	{
		...
		$bg_img = ROOT_PATH."/static/images/index_share_bg.jpg";
		...
		$msyh_path = ROOT_PATH."/static/fonts/msyh.ttf";
		$pingfang_path = ROOT_PATH."/static/fonts/PingFang_Bold.ttf";
		$pingfang_med_path = ROOT_PATH."/static/fonts/PingFang_Medium.ttf";
	...
	public function get_weindex_share_image($community_id,$wepro_qrcode,$avatar)
	{
		...
		foreach($community_goods_list as $goods)
		{
			...
			$price_arr = $this->get_goods_price($goods['id']);
	...
	public function get_goods_tags($label_id)
	{
		...
		$tag_info = M('lionfish_comshop_goods_tags')->field('id,tagname,type,tagcontent')->where( array('id' => $label_id) )->find();
	...
	public function get_min_time(){
		...
		//$sql = "select min(end_time) as rushtime from ".C('DB_PREFIX')."lionfish_comshop_good_common where ".$where."  ";

		$where = ' gc.begin_time <='. $now_time.' and gc.end_time > '.$now_time.' and g.id=gc.goods_id and g.grounding=1 and g.is_index_show=1 and gc.is_new_buy=0 and gc.is_spike_buy = 0  ';
		$sql = "select min(gc.end_time) as rushtime from " .C('DB_PREFIX')."lionfish_comshop_goods as g,".C('DB_PREFIX')."lionfish_comshop_good_common as gc where {$where}";
	...
	public function get_comming_goods_count(){
		...
		$count = M('lionfish_comshop_good_common')->where( " begin_time > {$now_time} " )->count();
	...
	public function get_weshare_image($goods_id)
	{
		
		$goods_info = M('lionfish_comshop_goods')->field('goodsname,price,productprice,sales,seller_count,total')->where( array('id' => $goods_id) )->find();
		...
		M('lionfish_comshop_good_common')->where( array('goods_id' => $goods_id) )->save( $up_data );
	...
	public function get_user_avatar($url, $member_id,$color=1)
	{
		...
		{ 
			...
			{  
				...
				M('lionfish_comshop_member')->where( array('member_id' => $member_id) )->save( array('wepro_qrcode' => $new_file) );
	...
	public function _get_goods_user_wxqrcode($goods_id,$member_id,$community_id)
	{
		...
		if($goods_id == 0)
		{
			...
			$gd_info = M('lionfish_comshop_goods')->field('type')->where( array('id'=>$goods_id) )->find();
	...
	public function _get_compare_zan_img($goods_img,$goods_title,$goods_price, $seller_count,$quantity)
	{
		...
		$bg_img = ROOT_PATH."/static/images/bg2.jpg";
	...
	**/
	public function user_fav_goods_toggle($goods_id, $member_id)
	{
		...
		{
			//删除
			M('lionfish_comshop_user_favgoods')->where( array('goods_id' => $goods_id,'member_id' => $member_id) )->delete();
			return 1;
		} else {
			//添加
			$data = array();
			...
			M('lionfish_comshop_user_favgoods')->add($data);
			return 2;
		}
	}
	public function check_goods_fav($goods_id, $member_id)
	{
		...
		$user_favgoods = M('lionfish_comshop_user_favgoods')->where( array('member_id' => $member_id, 'goods_id' => $goods_id) )->find();
	...
	**/
	public function get_goods_commission_info($goods_id,$member_id, $is_parse = false)
	{
		...
		$goods_commiss = M('lionfish_comshop_good_commiss')->where( array('goods_id' => $goods_id ) )->find();
	...
	**/
	...
	{
		$list = M('lionfish_comshop_goods')->field($fields)->where($where)->order($order)->limit($offset,$perpage)->select();
	...
	**/
	public function get_goods_images($goods_id, $limit =1)
	{
		...
		if($limit == 1)
		{
			$image_info = M('lionfish_comshop_goods_images')->where( array('goods_id' => $goods_id) )->order('id asc')->find();
			
			return $image_info;
			
		}else{
			$image_list = M('lionfish_comshop_goods_images')->where( array('goods_id' => $goods_id) )->order('id asc')->select();
	...
	**/
	public function fav_goods_count($goods_id)
	{
		...
		$total = M('lionfish_comshop_user_favgoods')->where( array('goods_id' => $goods_id) )->count();		
	...
	**/
	...
	{
		$fav_info = M('lionfish_comshop_user_favgoods')->where( array('goods_id' => $goods_id, 'member_id' => $member_id) )->find();		
	...
	**/
	public function get_goods_price($goods_id,$member_id = 0)
	{
		...
		$goods_info = M('lionfish_comshop_goods')->field('price as danprice,type,card_price')->where( array('id' => $goods_id))->find();
		
		if($goods_info['type'] =='pin')
		{
			
			$pin_goods_info = M('lionfish_comshop_good_pin')->field('pinprice,pin_count')->where( array('goods_id' => $goods_id ) )->find();		
			...
			if(!empty($pin_goods_info))
			{
				...
				$option_price_arr = M('lionfish_comshop_goods_option_item_value')->field('marketprice as dan_price')->where( array('goods_id' => $goods_id ) )->order('marketprice asc')->find();
				...
				$option_pinprice_arr = M('lionfish_comshop_goods_option_item_value')->field('pinprice as pin_price')->where( array('goods_id' => $goods_id) )->order('pinprice asc')->find();
			...
			//获取最低价格
			$option_price_arr = M('lionfish_comshop_goods_option_item_value')->field('id,marketprice as dan_price')
								->where( array('goods_id' => $goods_id) )->order('marketprice asc')->find();
			
			$max_option_price_arr = M('lionfish_comshop_goods_option_item_value')
			...
			$option_cardprice_arr = M('lionfish_comshop_goods_option_item_value')->field('id,card_price')->where( array('goods_id' => $goods_id ) )->order('card_price asc')->find();
	...
	public function get_goods_options($goods_id,$member_id =0)
	{
		...
		$goods_info = M('lionfish_comshop_goods')->field('goodsname,productprice')->where( array('id' => $goods_id ) )->find();
		
		$goods_common = M('lionfish_comshop_good_common')->field('is_mb_level_buy')->where( array('goods_id' => $goods_id ) )->find();
		
		
		$goods_option_query = M('lionfish_comshop_goods_option')->where( array('goods_id' => $goods_id) )->select();
    	...
    	if( !empty($goods_option_query) )
    	{
    		...
    		foreach ($goods_option_query as $goods_option) {
				...
				$goods_option_value_query = M('lionfish_comshop_goods_option_item')->where( array('goods_option_id' => $goods_option['id']) )->order('displayorder desc,id desc')->select();
			...
			$mult_item_list = M('lionfish_comshop_goods_option_item_value')->where( array('goods_id' => $goods_id) )->select();
	...
	**/
	public function get_goods_option_can_buy( $goods_id, $sku_str )
	{
		if( empty($sku_str) )
		{
			...
			$goods_option_mult_value = M('lionfish_comshop_goods_option_item_value')->where( array('option_item_ids' =>$sku_str,'goods_id' => $goods_id ) )->find();
	...
	public function get_goods_time_can_buy($goods_id)
	{
		...
		$goods_info = M('lionfish_comshop_good_common')->where( array('goods_id' => $goods_id) )->find();
	...
	**/
	...
	public function del_goods_mult_option_quantity($order_id,$option,$goods_id,$quantity,$type='1')
	{
		...
		$tp_goods = M('lionfish_comshop_goods')->field('total as quantity')->where( array('id' => $goods_id) )->find();
		
		$order_goods = M('lionfish_comshop_order_goods')->where( array('order_id' => $order_id, 'goods_id' => $goods_id ) )->find();
		
	    
		$option_list = M('lionfish_comshop_order_option')->where( array('order_id' => $order_id,'order_goods_id' => $goods_id) )->find();
	    ...
	    if($type == 1)
	    {
	        ...
	        //扣除库存
			$up_total_sql = "update ".C('DB_PREFIX')."lionfish_comshop_goods SET total = (total - " . (int)$quantity . ") where id={$goods_id} ";
			
			M()->execute($up_total_sql);
			
	        //销量增加
	       
		    $up_seller_count_sql = "update ".C('DB_PREFIX')."lionfish_comshop_goods SET seller_count = (seller_count + " . (int)$quantity . ") WHERE id = {$goods_id}";
	        
			M()->execute($up_seller_count_sql);
			
			//释放出reids占位，还有取消订单也要释放出redis占位---begin
			$order_info = M('lionfish_comshop_order')->field('member_id')->where( array('order_id' => $order_id) )->find();
	    ...
	    if(!empty($option))
	    {
	        if($type == 1)
	        {
				$up_sku_total_sql = "update ".C('DB_PREFIX')."lionfish_comshop_goods_option_item_value SET stock = (stock - " . (int)$quantity . ") where goods_id={$goods_id} and option_item_ids='{$option}' ";
	...
	**/
	public function get_goods_mixinfo($goods_id)
	{
		...
		$goods_info = M('lionfish_comshop_goods')->field('credit as points,type,codes as model')->where( array('id' => $goods_id) )->find();
		
		$commiss_info = M('lionfish_comshop_good_commiss')->where( array('goods_id' => $goods_id) )->find();
