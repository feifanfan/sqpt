...
class UserController extends CommonController {
	...
	public function set_default_address()
	{
		...
		M('ims_lionfish_comshop_address')->where( array('member_id' => $member_id ) )->save( $up_data );
			
		
		$up_data = array();
		$up_data['is_default'] = 1;
		
		M('ims_lionfish_comshop_address')->where( array('address_id' => $id ) )->save( $up_data );
	...
	public function group_orders()
	{
	    ...
	    foreach($list as $key => $val)
	    {
			...
			$first_tuan = M('lionfish_comshop_pin_order')->where( array('pin_id' => $val['pin_id'] ) )->order('id asc')->find();
	...
	public function sub_user_shop()
	{
		...
		//------
		$has_shopinfo =  M('lionfish_comshop_member_shopinfo')->where( array('member_id' => $member_id ) )->find();
		...
		if( !empty($has_shopinfo)  )
		{
			...
			M('lionfish_comshop_member_shopinfo')->add($data);
	...
	public function user_index_shareqrcode()
	{
		...
		if( $load_new )
		{
			...
			$member_info = M('lionfish_comshop_member')->field('avatar,username,wepro_qrcode')
	...
	public function myvoucherlist()
	{
        ...
        $list = M('lionfish_comshop_coupon_list')->where($condition)->order('add_time desc')->limit($offset,$pre_page)->select();
...

        {
			...
			$coupon = M('lionfish_comshop_coupon')->field('catid')->where( array('id' => $couponid) )->find();
...

	{
		...
		if($order_refund['order_goods_id'] > 0)
		{
			$order_refund_history = M('lionfish_comshop_order_refund_history')->where( array('type' => 2, 'order_goods_id' => $order_refund['order_goods_id'], 'order_id' =>$order_id ) )->order('addtime asc')->find();
			
		}else{
			
			$order_refund_history = M('lionfish_comshop_order_refund_history')->where( array('type' => 2,'order_id' => $order_id) )->order('addtime asc')->find();
		}
		...
		if($order_refund['order_goods_id'] > 0)
		{
			...
			$order_refund_historylist = M('lionfish_comshop_order_refund_history')->where( array('order_goods_id' => $order_refund['order_goods_id'],'order_id' => $order_id) )->order('addtime asc')->select();
...

	{
		...
		M('lionfish_comshop_order_refund')->where( array('ref_id' => $ref_id) )->save(  array('state' => 5) );
		...
		if($order_refund['order_goods_id'] > 0)
		{
			...
			M('lionfish_comshop_order_goods')->where( array('order_goods_id' => $order_refund['order_goods_id']) )->save( array('is_refund_state' => 0) );
			
			if($order_info['order_status_id'] == 12)
			{
				//检测  last_refund_order_status_id
				
				$order_gd_info = M('lionfish_comshop_order_goods')->where( array('is_refund_state' => 1, 'order_id' => $order_id ) )->find();
				
				if( empty($order_gd_info) )
				{
					if($order_info['last_refund_order_status_id'] > 0)
					{
						M('lionfish_comshop_order')->where(  array('member_id' => $member_id, 'order_id' => $order_id) )->save( array('order_status_id' => $order_info['last_refund_order_status_id'] ) );
					}else{
						M('lionfish_comshop_order')->where(  array('member_id' => $member_id, 'order_id' => $order_id) )->save( array('order_status_id' => 4) );
					}
				}
			}
	...
	public function refund_sub()
	{
		...
		if( !empty($order_goods_id) && $order_goods_id > 0 )
		{
			...
			$order_goods_list = M('lionfish_comshop_order_goods')->where( array('order_id' => $order_id, 'is_refund_state' => 0) )->select();
		...
		//	M('lionfish_comshop_order_history')->add($order_history);
			...
			{	
				...
				M('lionfish_comshop_order_goods')->where( array('order_id' => $order_id ) )->save( array('is_refund_state' => 1) );
			}
	...
	public function get_order_money()
	{
		...
		if( !empty($order_goods_id) && $order_goods_id > 0 )
		{
			...
			$order_goods_all = M('lionfish_comshop_order_goods')->where( array('order_goods_id' => $order_goods_id,'order_id' => $order_id) )->select();		
			...
			$order_goods_all = M('lionfish_comshop_order_goods')->where( array('order_id' => $order_id, 'is_refund_state' => 0) )->select();
	...
	public function getaddress()
	{
		...
		$address_list = M('lionfish_comshop_address')->where( array('member_id' => $member_id) )->order('is_default desc , address_id desc')->select();
		...
		foreach( $address_list as $key => $val )
		{
			...
			$province_info = M('lionfish_comshop_area')->field('name')->where( array('id' => $val['province_id']) )->find();
			
			
			$city_info = M('lionfish_comshop_area')->field('name')->where( array('id' => $val['city_id']))->find();
			
			$country_info = M('lionfish_comshop_area')->field('name')->where( array('id' => $val['country_id'] ) )->find();
	...
	**/
	public function collect_subscriptmsg()
	{
		...
		{
			$info = M('lionfish_comshop_subscribe')->where( array('member_id' => $member_id, 'type' => $type ) )->find();
			...
			M('lionfish_comshop_subscribe')->add( $ins_data );
...

	{
		...
		$order_goods = M('lionfish_comshop_order_goods')->where( array('order_id' =>$order_id ,'goods_id' => $goods_id) )->find();
		
		
		$order_option_list =M('lionfish_comshop_order_option')->where( array('order_goods_id' => $order_goods['order_goods_id']) )->select();
							
							
	    $order_goods['goods_images'] = tomedia($order_goods['goods_images']);  

		
		$goods_filed = M('lionfish_comshop_goods')->field('productprice')->where( array('id' => $order_goods['goods_id']) )->find();
...
//		M('goods')->field('image')->where( array('goods_id' => ) )->find();
...

	{
		...
		$order_goods = M('lionfish_comshop_order_goods')->field('name,goods_images')->where( array('goods_id' => $goods_id,'order_id' => $order_id) )->find();
			
		$order_info = M('lionfish_comshop_order')->field('order_num_alias')->where( array('order_id' => $order_id)  )->find();		
			
		$member_info = M('lionfish_comshop_member')->field('username as uname , avatar')->where( array('member_id' => $member_id) )->find();
		...
		M('lionfish_comshop_order_comment')->add($data);
	  ...
	  // $goods_info = M('goods')->field('store_id')->where( array('goods_id' => $goods_id) )->find();
	   ...
	   $comment_all_order = true;
		...
		$order_goods_all =	M('lionfish_comshop_order_goods')->field('goods_id')->where( array('order_id' => $order_id) )->select();
						
		foreach($order_goods_all as $val)
		{
			
			$order_comment = M('lionfish_comshop_order_comment')->field('comment_id')->where( array('goods_id' => $val['goods_id'],'order_id' =>$order_id) )->find();
	...
	**/
	public function get_user_integral_flow()
	{
		...
		$list = M('lionfish_comshop_member_integral_flow')->where(" member_id = ".$member_id."  ")->order('id desc')->limit($offset,$per_page)->select();
		...
		if( !empty($list) )
		{
			foreach($list as &$value)
			{
				...
				{
					$od_info = M('lionfish_comshop_order')->field('order_num_alias')->where( array('order_id' => $value['order_id'] ) )->find();		
					...
					$order_goods_info = M('lionfish_comshop_order_goods')->field('name')->where( array('order_goods_id' => $value['order_goods_id']) )->find();
	...
	public function get_user_charge_flow()
	{
		...
		if( !empty($list) )
		{
			foreach($list as &$value)
			{
				...
				{
					$od_info = M('lionfish_comshop_order')->field('order_num_alias')->where( array('order_id' => $value['trans_id'] ) )->find();
	...
	**/
    ...
    {   
		...
		if($phoneNumber){
			...
			M('lionfish_comshop_member')->where( array('member_id' => $member_id ) )->save($param);
   	...
   	public function decryptData($encryptedData, $iv, $token)
    {
		...
		$session_info = M('lionfish_comshop_weprogram_token')->where( array('token' => $token ) )->find();
	...
	public function applogin_do()
	{
		...
		$member_info = M('lionfish_comshop_member')->where( array('we_openid' => $openid) )->find();
		
		if( !empty($unionid) && empty($member_info) )
		{
			$member_info = M('lionfish_comshop_member')->where( array('unionid' => $unionid) )->find();
		}
		...
		if(!empty($member_info) )
		{
			 ...
			 M('lionfish_comshop_member')->where( array('member_id' => $member_info['member_id'] ) )->save(  $data );
			...
			**/			
			...
			$old_token_info = M('lionfish_comshop_weprogram_token')->where( array('member_id' => $member_info['member_id'] ) )->find();
			
			if( !empty($old_token_info) )
			{
				M('lionfish_comshop_car')->where( array('token' => $old_token_info['token'] ) )->delete();
			}
			
			
			M('lionfish_comshop_weprogram_token')->where( array('member_id' => $member_info['member_id']) )->delete();
			...
			M('lionfish_comshop_weprogram_token')->add($weprogram_token_data);
			...
			if( $share_id > 0 )
			{
				...
				if( !empty($commiss_level) && $commiss_level > 0)
				{
					...
					$share_member_info = M('lionfish_comshop_member')->field('comsiss_flag,comsiss_state')->where( array('member_id' => $share_id ) )->find();
			...
			$member_id = M('lionfish_comshop_member')->add( $data );
			...
			if( $share_id > 0 )
			{
				...
				if( !empty($commiss_level) && $commiss_level > 0)
				{
					...
					$share_member_info = M('lionfish_comshop_member')->field('is_writecommiss_form,comsiss_flag,comsiss_state')->where( array('member_id' =>$share_id ) )->find();
					
					if( !empty($share_member_info) )
					{
						if(  $share_member_info['comsiss_flag'] == 1  && $share_member_info['comsiss_state'] == 1 )
						{
							M('lionfish_comshop_member')->where( array('member_id' => $member_id ) )->save( array('agentid' => $share_id ) );
						}
					}
		...
		if($open_danhead_model == 1 && $member_id > 0 )
		{
			...
			$default_head = M('lionfish_community_head')->field('id')->where( array('is_default' => 1) )->find();
	...
	**/
	public function get_copyright()
	{
		...
		$info = M('lionfish_comshop_config')->field('value')->where( array('name' => 'footer_copyright_desc') )->find();
	...
	public function get_account_money(){
		...
		$chargetype_list = M('lionfish_comshop_chargetype')->order( 'id asc' )->select();
