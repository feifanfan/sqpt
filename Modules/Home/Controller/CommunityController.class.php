...
class CommunityController extends CommonController {
	...
	**/
	public function get_community_bind_member_qrcode()
	{
		...
		if( empty($community_memberbind_qrcode) )
		{
			...
			$hexiao_data = unserialize($community_memberbind_qrcode['hexiao_qrcod']);
	...
	**/
	...
	public function get_community_member_orderlist()
	{
		...
		if( isset($gpc['searchKey']) && !empty($gpc['searchKey']) )
		{
		 ...
		 $sql = "select h.member_id,h.id ,m.avatar, m.username ,("."select count(order_id) from ".C('DB_PREFIX')."lionfish_comshop_order  
								where  order_status_id =4 and delivery!='express' and member_id=m.member_id and head_id= ".$community_info['id'].") as m_count 
		...
		foreach($list as $key => $val)
		{
			...
			$order_count = M('lionfish_comshop_order')->where( array('head_id' => $community_info['id'],'delivery'=>array('neq','express'),'member_id' => $val['member_id'],'order_status_id' => 4) )->count();					
	...
	public function get_community_info()
	{
		...
		//待配送 1  1   
		$wait_send_count =  M('lionfish_comshop_order')->where( array('order_status_id' => 1, 'head_id' => $head_id)  )->count();
	...
	**/
	public function get_head_distribute_info()
	{
		...
		if( empty($level_count1) || $level_count1 == 0 )
		{
			...
			$level_sql2 =" select count(1) from ".C('DB_PREFIX').
						"lionfish_community_head  where  
							agent_id in (select id from ".C('DB_PREFIX')."lionfish_community_head  
	...
	**/
	...
	public function get_apply_page()
	{
	    ...
	    if(!empty($info['value'])){
	    	echo json_encode( array('code' => 0, 'data' => htmlspecialchars_decode($info['value'])) );
			die();
	    }else{
	    	echo json_encode( array('code' => 1 ));
			die();
