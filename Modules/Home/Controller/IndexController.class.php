...
class IndexController extends CommonController {
	...
	{
		$item = M('lionfish_comshop_config')->where( array('name' => 'shop_index_share_title') )->find();
	...
	public function test_sql()
	{
		...
		/**
		$res =  check_db_field_exist( 'oscshop_lionfish_community_head_commiss_order','fen_type' );
		if( !$res )
		{
			$sql_content ="
	...
	public function test()
	{
		...
		D('Home/Pin')->insertNotifyOrder($order_id);
		
		$is_pin_success = D('Home/Pin')->checkPinSuccess($pin_order['pin_id']);
		
		
		var_dump($is_pin_success, $pin_order['pin_id']);
		die();
		...
		$res=$redis->llen('goods_1');
		
		$count=10;
		for($i=0;$i<$count;$i++){
			$redis->lpush('goods_1',1);
		}
		
		//$ret = $redis->lrem('city', 'guangzhou', -2);
		
		var_dump($res);die();
		//https://blog.csdn.net/nuli888/article/details/51865401  参考这里就可以解决高并发 的问题
	...
	public function index_info()
	{
		...
		$postion = M('lionfish_community_head')->field('lat,lon')->where( array('id' => $communityId) )->find();
		...
		/**  调用滑动广告 **/
		$slider_list = M('lionfish_comshop_adv')->where( array('enabled' => 1,'type' => 'slider') )->order('displayorder desc, id desc')->select();
		...
		$notice_list = M('lionfish_comshop_notice')->where( array('enabled' => 1) )->order('displayorder desc, addtime desc')->select();
		
		/**  调用导航图标  **/
		
		$nav_list = M('lionfish_comshop_adv')->where( array('enabled' => 1,'type' => 'nav') )->order('displayorder desc, id desc')->select();
		...
		**/
		...
		$cube =  M('lionfish_comshop_cube')->where( array('enabled' => 1) )->order('displayorder desc, addtime desc')->select();		
	...
	{
		$list = M('lionfish_comshop_navigat')->field('id,navname,thumb,link,type,appid')->where("enabled=1")->order('displayorder desc')->select();
	...
	public function switch_history_community(){
		...
		//删除 community_id
		M('lionfish_comshop_car')->where( "token='{$token}' and community_id!='{$head_id}' " )->delete();
	...
	**/
		...
		if( !empty($head_id) && $head_id >0 )
		{
			if($gid == 0){
				...
				$sql_goods_ids = "select pg.goods_id from ".C('DB_PREFIX')."lionfish_community_head_goods as pg,"
        	           ...
        	           where  pg.goods_id = g.goods_id  and g.cate_id = {$gid} and pg.head_id = {$head_id}  order by pg.id desc ";
			...
			if($gid == 0){
				...
				$goods_ids_nolimit_sql = "select pg.id from ".C('DB_PREFIX')."lionfish_comshop_goods as pg,"
                        .C('DB_PREFIX')."lionfish_comshop_goods_to_category as g  
        	           where  pg.id = g.goods_id and g.cate_id = {$gid} and pg.is_all_sale=1 ";
	...
	public function get_index_category()	
	{
		...
		$hot_list = M('lionfish_comshop_goods_category')->where( array('is_hot' => 1) )->order('sort_order desc')->select();
	...
	public function get_index_notice()
	{
		...
		$list = M('lionfish_comshop_notice')->where( array('enabled' => 1) )->order('displayorder desc, addtime desc')->limit(1)->select();		
	...
	public function load_new_buy_goodslist()
	{
		...
		if( !empty($head_id) && $head_id >0 )
		{
			...
			$sql_goods_ids = "select pg.goods_id from ".C('DB_PREFIX')."lionfish_community_head_goods as pg,"
	                        .C('DB_PREFIX')."lionfish_comshop_good_common as g where pg.goods_id = g.goods_id and g.is_new_buy = 1 and pg.head_id = {$head_id} order by pg.id desc ";
            $goods_ids_arr = M()->query($sql_goods_ids);
			...
			$goods_ids_nolimit_ids = "select pg.id from ".C('DB_PREFIX')."lionfish_comshop_goods as pg,"
	                        .C('DB_PREFIX')."lionfish_comshop_good_common as g where pg.id = g.goods_id and g.is_new_buy = 1 and pg.is_all_sale=1  order by pg.id desc ";
            $goods_ids_nolimit_arr = M()->query($goods_ids_nolimit_ids);
	...
	public function load_cate_goodslist()
	{
		...
		$parent_cate = M('lionfish_comshop_goods_category')->field('id,banner,name')->where( array('is_show' => 1,'cate_type' =>'normal','id' => $pid ) )->find();
		
	    if($parent_cate){
	    	
			$cate_info = M('lionfish_comshop_goods_category')->field('id,banner,name,logo')->where( array('is_show' => 1,'cate_type' =>'normal','pid' => $parent_cate['id'] ) )->order('sort_order desc, id desc')->select();			
			...
			foreach ($cateArr as $key => $val) {
				...
				if( !empty($head_id) && $head_id >0 )
				{
					...
					$goods_ids_nolimit_sql = "select pg.id from ".C('DB_PREFIX')."lionfish_comshop_goods as pg,"
		                    .C('DB_PREFIX')."lionfish_comshop_goods_to_category as g where pg.id = g.goods_id and g.cate_id={$gid} 
					...
					$goods_ids_nohead_sql = "select pg.id from ".C('DB_PREFIX')."lionfish_comshop_goods as pg," .C('DB_PREFIX')."lionfish_comshop_goods_to_category as g 
