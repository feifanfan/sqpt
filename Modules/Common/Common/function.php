...
function all_qiniu()
{
	...
	//qiniu_accesskey 
	$accessKey_arr =  M('lionfish_comshop_config')->where( array('name' => 'qiniu_accesskey') )->find();
	$secretKey_arr =  M('lionfish_comshop_config')->where( array('name' => 'qiniu_secretkey') )->find();
	$bucket_arr =  M('lionfish_comshop_config')->where( array('name' => 'qiniu_bucket') )->find();
...
function tpl_form_field_multi_image3($name, $value = array(), $options = array())
{
	...
	if (isset($options['dest_dir']) && !empty($options['dest_dir'])) {
		if (!preg_match('/^\\w+([\\/]\\w+)?$/i', $options['dest_dir'])) {
			exit('图片上传目录错误,只能指定最多两级目录,如: "lionfish_comshop","Snailfish_shop/a1"');
		}
	}
	...
	if (!defined('TPL_INIT_MULTI_IMAGE')) {
		$s = "\r\n<script type=\"text/javascript\">\r\n\tfunction uploadMultiImage(elm) {\r\n\t\tvar name = \$(elm).next().val();\r\n\t\tutil.image( \"\", function(urls){\r\n\t\t\t\$.each(urls, function(idx, url){\r\n\t\t\t\t\$(elm).parent().parent().next().append('<div class=\"multi-item\"><img onerror=\"this.src=\\'/static/images/nopic.png\\'; this.title=\\'图片未找到.\\'\" src=\"'+url.url+'\" class=\"img-responsive img-thumbnail\"><input type=\"hidden\" name=\"'+name+'[]\" value=\"'+url.attachment+'\"><em class=\"close\" title=\"删除这张图片\" onclick=\"deleteMultiImage(this)\">×</em></div>');\r\n\t\t\t});\r\n\t\t}, " . json_encode($options) . ");\r\n\t}\r\n\tfunction deleteMultiImage(elm){\r\n\t\trequire([\"jquery\"], function(\$){\r\n\t\t\t\$(elm).parent().remove();\r\n\t\t});\r\n\t}\r\n</script>";
	...
	if (is_array($value) && (0 < count($value))) {
		foreach ($value as $row) {
			...
			$s .= "\r\n<div class=\"multi-item\">\r\n\t<img src=\"" . ($row['thumb']) . "\" onerror=\"this.src='/static/images/nopic.png'; this.title='图片未找到.'\" class=\"img-responsive img-thumbnail\">\r\n\t<input type=\"hidden\" name=\"" . $name . '[]" value="' . $row['image'] . "\" >\r\n\t<em class=\"close\" title=\"删除这张图片\" onclick=\"deleteMultiImage(this)\">×</em>\r\n</div>";
		}
	}
...
function save_image_to_txyun($file_path,$new_file_name)
{
	...
	try {
		$result = $cosClient->upload(
			$bucket = D('Home/Front')->get_config_by_name('tx_bucket'), //格式：BucketName-APPID 
			$key = $new_file_name,
		...
		// 请求失败
		echo($e);
		return array('code' => 1, 'msg' => '上传失败'.$e );
	}
}
...
{
	if (empty($default)) {
		$default = '/static/images/nopic.png';
	}
	$val = $default;
	...
	if (!empty($value)) {
		...
		$val = '/static/images/default-pic.jpg';
	}
	...
	if (!defined('TPL_INIT_IMAGE')) {
		$s = "\r\n\t\t<script type=\"text/javascript\">\r\n\t\t\tfunction showImageDialog(elm, opts, options) {\r\n\t\t\t\trequire([\"util\"], function(util){\r\n\t\t\t\t\tvar btn = \$(elm);\r\n\t\t\t\t\tvar ipt = btn.parent().prev();\r\n\t\t\t\t\tvar val = ipt.val();\r\n\t\t\t\t\tvar img = ipt.parent().next().children();\r\n\t\t\t\t\toptions = " . str_replace('"', '\'', json_encode($options)) . ";\r\n\t\t\t\t\tutil.image(val, function(url){\r\n\t\t\t\t\t\tif(url.url){\r\n\t\t\t\t\t\t\tif(img.length > 0){\r\n\t\t\t\t\t\t\t\timg.get(0).src = url.url;\r\n\t\t\t\t\t\t\t\timg.closest(\".input-group\").show();\r\n\t\t\t\t\t\t\t}\r\n\t\t\t\t\t\t\tipt.val(url.attachment);\r\n\t\t\t\t\t\t\tipt.attr(\"filename\",url.filename);\r\n\t\t\t\t\t\t\tipt.attr(\"url\",url.url);\r\n\t\t\t\t\t\t}\r\n\t\t\t\t\t\tif(url.media_id){\r\n\t\t\t\t\t\t\tif(img.length > 0){\r\n\t\t\t\t\t\t\t\timg.get(0).src = \"\";\r\n\t\t\t\t\t\t\t}\r\n\t\t\t\t\t\t\tipt.val(url.media_id);\r\n\t\t\t\t\t\t}\r\n\t\t\t\t\t}, options);\r\n\t\t\t\t});\r\n\t\t\t}\r\n\t\t\tfunction deleteImage(elm){\r\n\t\t\t\trequire([\"jquery\"], function(\$){\r\n\t\t\t\t\t\$(elm).prev().attr(\"src\", \"../addons/lionfish_comshop/static/images/default-pic.jpg\");\r\n\t\t\t\t\t\$(elm).parent().prev().find(\"input\").val(\"\");\r\n\t\t\t\t});\r\n\t\t\t}\r\n\t\t</script>";
...
{
	$ret = array('status' => $status, 'result' => $status == 1 ? array('url' => '') : array());
...
{
	$water_info = M('config')->where( array('name' => 'WATER_BG') )->find();
...
function pay_token($key_name){
	$key='lionfish'.rand(100000, 999999);
	$token=md5($key);
	...
	if(!empty($template_msg_order))
	{
		foreach($template_msg_order as $msg_order)
		{
			...
			else if($msg_order['type'] == 2)
			{
				...
				//$form_id
				$member_info = M('member')->field('member_id')->where( array('we_openid' => $msg_order['open_id']) )->find();
				
				$member_formid_info = M('member_formid')->where( array('member_id' => $member_info['member_id'],'state' =>0) )->find();
